...
void dgemm_edge(int lda, int m, int n, int k, double *A, double *B, double* C)
{
  ...
  // This requires accessing the whole matrix B, so there is still room for optimization
  if (m % 2)
  {
    // Need to update the last row
    for(c = 0; c < n; c++)
    {
      for(p = 0; p < k; p ++)
      {
        C[m-1+c*lda] += A[m-1+p*lda]*B[p+c*lda];
      }
    }
    ...
    if (n % 2)
    {
      for(r = 0; r < m-1; r++)
      {
        for(p = 0; p < k; p ++)
        {
          C[r+(n-1)*lda] += A[r+p*lda]*B[p+(n-1)*lda];
        }
      }
    }
  }else if (n % 2)
...
{
  if (DEBUG) printf("Doing block m = %d, n = %d, k = %d\n", m, n, k);
  int i, j;
  double packedA[m*k], packedB[BLOCK_SIZE*nb]; 

  for (j=0; j<n/4*4; j+=4){        /* Loop over the columns of C, unrolled by 4 */
    if (need_to_packB)
      copyB(k, B+j*lda, lda, &packedB[j*k]);
    for (i=0; i<m/8*8; i+=8){        /* Loop over the rows of C */
  ...
  else if (m%8!=0 && n%4 != 0)
  {
    ...
    dgemm_edge(lda, m-row_index, n, k, A+row_index, B, C+row_index);
    dgemm_edge(lda, row_index, n-col_index, k, A, B+lda*col_index, C+lda*col_index);
  }
